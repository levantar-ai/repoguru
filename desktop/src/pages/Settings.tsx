...
export function Settings() {
  ...
  const maskedToken = token
    ? `${token.slice(0, 7)}${'*'.repeat(Math.min(token.length - 7, 30))}`
    : '';
    ...
    <div className="p-8 max-w-3xl mx-auto">
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-lg border border-gray-800 bg-gray-900/50 p-4">
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            {isSet && (
              <span className="text-[10px] text-gray-600 ml-1">
                (encrypted via{' '}
                {'secureStore' in (window.repoGuru || {}) ? 'safeStorage' : 'localStorage'})
              </span>
            )}
          </div>
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1">
              {isSet
                ? 'Replace token'
                : 'Personal Access Token (for org scans, GitHub autocomplete)'}
            </label>
            ...
            <p className="text-[10px] text-gray-600 mt-1">
              Stored securely. Used for GitHub API calls (org scans, repo autocomplete). Needs{' '}
              <code className="text-gray-500">repo</code> scope.
            </p>
          </div>
        </section>
        ...
        <div className="flex items-center gap-3">
          ...
          {saved && <span className="text-xs text-green-400 animate-fade-in-up">Saved!</span>}
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-gray-600 mt-0.5">
                Display demo repository cards on the Dashboard
              </p>
            </div>
            <button
              onClick={() => {
