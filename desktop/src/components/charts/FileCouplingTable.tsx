...
export function FileCouplingTable({ coupling }: Props) {
  ...
  return (
    <ChartCard
      title="File Coupling"
      subtitle="Files frequently changed together"
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {top.map((c, i) => (
              ...
              >
                <td
                  className="py-1.5 text-[var(--color-text-secondary)] font-mono truncate max-w-[250px]"
                  title={c.file_a}
                >
                  {c.file_a}
                </td>
                ...
                <td className="text-right py-1.5">
                  <span
                    className={`${c.coupling_pct >= 80 ? 'text-red-400' : c.coupling_pct >= 50 ? 'text-yellow-400' : 'text-[var(--color-text)]'}`}
                  >
