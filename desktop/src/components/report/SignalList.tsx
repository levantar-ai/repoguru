      ...
      {signals.map((signal) => (
        <li key={signal.name} className="flex items-start gap-2 text-xs">
          ...
          <div className="flex-1 min-w-0">
            <span className={signal.found ? 'text-gray-300' : 'text-gray-500'}>
              {signal.name}
            </span>
            {signal.details && (
              <span className="text-gray-600 ml-1">({signal.details})</span>
            )}
            {signal.points > 0 && (
              <span className="text-gray-600 ml-1">+{signal.points}pts</span>
            )}
          </div>
        </li>
      ))}
