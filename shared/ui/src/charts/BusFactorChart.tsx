...
/**
 * D3 Lorenz curve with the browser app's full annotation set: equality
 * line, 50% threshold, bus-factor vertical marker, dots on the curve.
 * Renders bare (legend chips above the SVG) so a host's `<ChartSection>`
 * provides the surrounding card chrome.
 */
export function BusFactorChart({ busFactor, height = 350 }: BusFactorChartProps) {
  const render = useCallback(
    (svg: SVGSVGElement, width: number, h: number) => {
  ...
  return (
    <div>
      ...
      <D3Container render={render} height={height} />
    </div>
  );
}
