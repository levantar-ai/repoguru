    ...
    <div className="w-full px-8 lg:px-12 xl:px-16 py-10 sm:py-16">
      ...
      {hasResults && state.analysis && (
        <div className="space-y-6">
          ...
          {/* Radar Health */}
          {state.canonical?.health?.radarMetrics &&
            state.canonical.health.radarMetrics.length > 0 && (
              <ChartSection title="Repository Health">
                <RadarHealthCard radarMetrics={state.canonical.health.radarMetrics} />
              </ChartSection>
            )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {state.canonical?.health?.busFactor && (
              <ChartSection title="Bus Factor (Lorenz Curve)">
                <BusFactor busFactor={state.canonical.health.busFactor} />
              </ChartSection>
            )}
          </div>
