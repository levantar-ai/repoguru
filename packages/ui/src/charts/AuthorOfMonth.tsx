...
import type { AuthorOfPeriod } from './legacyTypes.js';

interface Props {
  readonly authorOfMonth: AuthorOfPeriod[];
}
...
export function AuthorOfMonth({ authorOfMonth }: Props) {
  const [sortKey, setSortKey] = useState<SortKey>('period');
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('desc');

  const sorted = useMemo(() => {
    const items = [...authorOfMonth];
    items.sort((a, b) => {
      const av = a[sortKey];
      const bv = b[sortKey];
  ...
  if (authorOfMonth.length === 0) return null;

  const maxCommits = Math.max(...authorOfMonth.map((e) => e.commits), 1);
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border">
            ...
            >
              Month <SortIcon active={sortKey === 'period'} dir={sortDir} />
            </th>
            <th
              className="text-left py-3 px-3 text-xs font-semibold text-text-secondary uppercase tracking-wider cursor-pointer hover:text-neon transition-colors"
        ...
        <tbody>
          {sorted.map((entry) => (
            ...
            >
              ...
              <td className="py-2.5 px-3 text-right tabular-nums text-text-muted hidden md:table-cell">
