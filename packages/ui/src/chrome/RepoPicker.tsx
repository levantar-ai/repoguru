  ...
  return (
    <div>
      ...
      {!hideAuthChrome && supportsGitHub && hasToken && ghRepos.length > 0 && (
        // Combobox combines the filter input + repo list under one
        // ARIA-correct keyboard model. Arrow keys move the highlight
        // in the right pane; Enter selects; aria-activedescendant
        // wired automatically. Replaces ~80 lines of hand-rolled focus
        // and selection state.
        <Combobox<GitHubRepoSummary | null>
          immediate
          value={null}
          onChange={(repo) => {
            if (repo) onChange(`${repo.owner}/${repo.repo}`);
          }}
        >
                ...
                {filteredRepos.length === 0 ? (
                  ...
                  filteredRepos.map((repo) => (
                    <ComboboxOption
                      ...
                      value={repo}
                      disabled={disabled}
                      className="w-full flex items-center gap-3 px-4 py-2.5 text-left cursor-pointer transition-colors disabled:opacity-50 border-b border-border/50 last:border-b-0 data-[focus]:bg-surface-hover"
                    >
