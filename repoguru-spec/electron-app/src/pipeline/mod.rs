...
pub fn run_pipeline(args: &ScanArgs) -> Result<(), ScanError> {
    ...
    let agg_result = aggregator::run_aggregator(result_rx, out, renames_enabled, &agg_log);
    ...
    if args.report.is_on() {
        ...
        generate_report(out, &metrics, &args.repo)
    ...
    log.log(&format!(
        "[done] Scan complete in {:.1}s ({} commits, {} file changes, output: {})",
